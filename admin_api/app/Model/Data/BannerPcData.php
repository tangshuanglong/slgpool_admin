...
class BannerPcData
{
     ...
     * @throws DbException
     */
    public static function get_pc_banner_list(int $page_index)
    {
        $per_num = config('per_page_num');
         $banner = DB::table('img_banner_pc')
            ->leftJoin('lang_type', 'lang_type.type_code', '=', 'img_banner_pc.lang_type')
            ->select('img_banner_pc.*', 'lang_type.type_name')
            ->orderBy('status', 'DESC')
        ...
        $data['info'] = $banner;
        $data['count'] = DB::table('img_banner_pc')->count();
        return $data;
    }
