...
class MyValidator {
	...
	public function auth_all_verify_code($uid, $params, $action)
    {
        ...
        if ($user_bind_info['google_validator'] == 1){
            ...
            if (!$res) {
                return MyQuit::returnMessage(MyCode::CAPTCHA_GOOGLE_ERROR, 'google verify code error');
            }
        }
        ...
        if ($verify_res === 'mobile') {
            $res = MyQuit::returnMessage(MyCode::CAPTCHA_MOBILE_ERROR, 'sms verify code error');
        }elseif ($verify_res === 'email') {
            $res = MyQuit::returnMessage(MyCode::CAPTCHA_EMAIL_ERROR, 'email verify code error');
        }elseif ($verify_res === false) {
            $res = MyQuit::returnMessage(MyCode::PARAM_ERROR, 'params error');
        }
        return $res;
    }
